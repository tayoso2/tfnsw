...
pavement_UP <- as.data.frame(all_p_nop) #plyr::rbind.fill(as.data.frame(all_p_nop),as.data.frame(all_p_nop2))
###
##
#
...
p_up_cost <- read_excel(("C:/Users/TOsosanya/Desktop/NSW/Newdata 2705/Pavements/Copy of Tactical Buildup-26.06.2020.xlsx"),range = "A1:D6", sheet = "Averages") %>% 
  filter(`Works Type` == "Unassigned Projects") %>% dplyr::select(`Total Cost`) %>% as.list()
...
get_unassigned_projects <- function(data, U.P, A.C, A.G){
  gup <- data %>%
    dplyr::select(`Project Name`,everything()) %>% 
...
up_names_id <- read_excel(("C:/Users/TOsosanya/Desktop/NSW/UVO/Final-NSW-Scen2.xlsx"),range = "A1:B681", sheet = "id") %>%
  #filter(`Asset Group` != "ITS asset management") %>% 
...
up_names_id_2 <- read_excel(("C:/Users/TOsosanya/Desktop/NSW/UVO/Final-NSW-Scen2.xlsx"),range = "A1:B681", sheet = "id") %>%
  #filter(`Asset Group` != "ITS asset management") %>% 
  dplyr::select(1:2) %>% 
  filter(grepl('Remaining ',`name`))
...
function4 <- function(up_names_id_2){
  ...
  up_names_id_3 <- up_names_id_2 %>% left_join(all_UP[,c("Asset Group","Project Name")], by = "Asset Group") %>% 
...
InB_all_p_InB <- all_UP %>% dplyr::select(`Project Name`) %>% 
  mutate(In_Backlog1 = 0,
         In_Backlog2 = 0,
         In_Backlog3 = 0,
         In_Backlog4 = 0,
         In_Backlog5 = 0,
...
InB_all_p_InB2$measure = gsub('[[:digit:]]+', '', InB_all_p_InB2$measure)
InB_all_p_InB2 <- InB_all_p_InB2 %>% 
  as.data.table()

# add solution 0 and then solution 1(selected)

InB_all_p_InB3 <- InB_all_p_InB2 %>% 
  mutate(solution = 0)
InB_all_p_InB4 <- InB_all_p_InB3 %>% 
  mutate(value = 0, solution = 1)

InB_all_p_InB5 <- rbind(InB_all_p_InB3,InB_all_p_InB4) %>% 
  rename(project_name = `Project Name`) %>% 
  inner_join(up_names_id[,c("project","project_name","selection.timing", "must")], by = "project_name") %>% 
  dplyr::select(project, project_name, measure, timestep, value, solution, selection.timing, must) %>% 
  distinct()


up_all_xx3 <- rbind(up_all_xx3,InB_all_p_InB5) 
