...
fmeca_post_intervention <- fmeca1_its3 %>% 
  # fmeca x conops
...
source("C:/Users/TOsosanya/Desktop/NSW/tfnsw/R_post_intervention/ITS NPI.R")
...
its_assets1 <- read_excel(paste0(path_i, "/ITS mastersheet.xlsx"), range = "A1:AJ3118" ,sheet = "original mastersheet")
its_assets_backlog_projects <- its_assets1 %>% dplyr::select(AssetID,	BacklogProjectName) %>% mutate(AssetID = as.character(AssetID))
fmeca_npi <- fmeca_npi %>%
  mutate(AssetID = ifelse(!is.na(TCS_ID),as.character(TCS_ID),as.character(LABEL))) %>% 
  inner_join(its_assets_backlog_projects,by = "AssetID") %>%
...
unique(fmeca_post_intervention$AssetID)
...
calc_det_after_intervention <- function(fmeca_post_intervention_edited_split){
  ...
  output.final2 <- output.final %>% 
    ...
    dplyr::select(`ObjectID`:FMECA_Political_CONOPS) %>% as.data.table()
  return(output.final2)
}



ITS_post_int <- calc_det_after_intervention(fmeca_post_intervention_edited_split)



# write results

openxlsx::write.xlsx(ITS_post_int, file = paste0(path_i, "/ITS results post_intervention.xlsx"))
