...
all_p <- plyr::rbind.fill(assgnd_p_p,assgnd_p_c,assgnd_p_b,assgnd_p_i) %>% 
  select(AssetID,`Asset Group`,`Asset Category`,`Project Name`,POF1:POF10_FMECA_Political_CONOPS,
         `selection.timing21-23`,backlog.selection.timing,must,`Total Cost`) %>% 
...
pavement_UP1 <- all_p %>%  
  distinct() %>% 
  filter(must == "TRUE",!is.na(`selection.timing21-23`))
pavement_UP2 <- all_p %>%  
  distinct() %>% 
  filter(must == "TRUE",is.na(`selection.timing21-23`)) %>% 
  as.data.table() %>% 
  mutate(selector = ifelse(must == "TRUE",rep(1:20),0)) %>% 
  filter(selector != "1") %>% 
  select(-selector) %>% as_tibble()
...
pavement_UP <- plyr::rbind.fill(pavement_UP1,pavement_UP2) 
...
source("C:/Users/TOsosanya/Desktop/NSW/tfnsw/Merging Results Scen 4 non projects test.R")
...
all_complete_Scen4 <- all_complete_Scen4 %>% 
  ...
  mutate(must = case_when(selection.timing == "yyynnnnnnn" ~ "TRUE", 
                          selection.timing == "nyyynnnnnn" ~ "TRUE",
                          selection.timing == "nnyyynnnnn" ~ "TRUE",
                          TRUE ~ "FALSE"))
