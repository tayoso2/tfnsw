...
corridor_culvert4 <- corridor_culvert3 %>% 
  ...
  mutate(POF0 = 0) %>%  # apply the flat deterioration >> changed
...
corridor_slope3 <- corridor_slope2 %>%
  mutate(CONSTRUCTION_YEAR = CT) %>% # random generated construction_year >> changed
...
fmeca_post_intervention <- fmeca3 %>% 
  # fmeca x conops
...
source("C:/Users/TOsosanya/Desktop/NSW/tfnsw/R_post_intervention/Corridors NPI.R")
...
corridor_assets1 <- read_excel(paste0(path_c, "/Corridor mastersheet.xlsx"), range = "A1:BM9138" ,sheet = "original mastersheet")
corridor_assets_backlog_projects <- corridor_assets1 %>% dplyr::select(`Asset Number`,	BacklogProjectName) %>% mutate(`Asset Number` = as.character(`Asset Number`))
fmeca_npi <- fmeca_npi %>%
  mutate(`Asset Number` = as.character(`Asset Number`)) %>% 
  inner_join(corridor_assets_backlog_projects,by = "Asset Number") %>%
  mutate(`Project Name` = ifelse(is.na(`Project Name`),BacklogProjectName,`Project Name`))
fmeca_post_intervention <- fmeca_post_intervention %>%
  mutate(`Asset Number` = as.character(`Asset Number`)) %>%  
  inner_join(corridor_assets_backlog_projects,by = "Asset Number") %>%
  mutate(`Project Name` = ifelse(is.na(`Project Name`),BacklogProjectName,`Project Name`)) #%>% 
#dplyr::select(-c(BacklogProjectName,AssetID))
unique(fmeca_npi$`Project Name`)
unique(fmeca_post_intervention$`Project Name`)
unique(fmeca_post_intervention$`Asset Number`)
...
fmeca_post_intervention_pofrenamed <- fmeca_post_intervention %>% 
...
fmeca_pi_pofmerged <- cbind(fmeca_post_intervention_pofrenamed,fmeca_npi[,columns_AN_POF])
...
corridor_post_int <- calc_det_after_intervention(fmeca_post_intervention_edited_split)

# testing

fmeca_post_intervention_edited %>% filter(selection.date.year == 3)
corridor_post_int %>% filter(`Project Name` == "Remediation of culvert 415006")

fmeca_post_intervention_edited %>% filter(selection.date.year == 10)
corridor_post_int %>% filter(`Project Name` == "Remediation of culvert 429012")

fmeca_post_intervention_edited %>% filter(selection.date.year == 1)
corridor_post_int %>% filter(`Project Name` == "Remediation of culvert 446072")

#### ----------------------------------------

openxlsx::write.xlsx(corridor_post_int, file = paste0(path_c, "/Corridor results post_intervention.xlsx"))
