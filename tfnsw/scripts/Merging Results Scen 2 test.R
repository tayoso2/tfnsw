...
# select the projects with project name only
...
all_p_nop <- plyr::rbind.fill(as.data.frame(all_p_nop),as.data.frame(all_p_nop2))
###
##
#
...
##
###
pnames_id <- read_excel(("C:/Users/TOsosanya/Desktop/NSW/UVO/Final-NSW-Scen2.xlsx"),range = "A1:B673", sheet = "id") %>%
  #filter(`Asset Group` != "ITS asset management") %>%
  select(1:2)
###
##
...
##
###
pnames_id_2 <- read_excel(("C:/Users/TOsosanya/Desktop/NSW/UVO/Final-NSW-Scen2.xlsx"),range = "A1:B673", sheet = "id") %>%
  #filter(`Asset Group` != "ITS asset management") %>% 
...
source("C:/Users/TOsosanya/Desktop/NSW/tfnsw/Merging Results Scen 2 non projects test.R")
...
all_complete_project <- all_complete %>% select(project,project_name,must,Asset_Group, Bucket) %>%
  rename(name = project_name) %>%
  rename(Zone = Asset_Group) %>%
  rename(Region = Bucket) %>% select(project,name, must,Region, Zone) %>% 
  distinct()
all_complete_solution <- all_complete %>% select(project,solution,selection.timing) %>% 
  filter(solution == 1) %>% 
  mutate(time.to.benefits = 1) %>% distinct()
all_complete_measures <- all_complete %>% select(project:solution)
...
openxlsx::write.xlsx(all_complete, file = "C:/Users/TOsosanya/Desktop/NSW/Newdata 2705/Pavements/Pavement UVO 3 ids Scen2.xlsx")
...
# scenario 4
all_complete <- rbind(all_xx3,up_all_xx3)
all_complete_Scen4 <- all_complete %>% 
  mutate(value = ifelse(grepl('Total_Cost',`measure`),value*0.95,value)) %>% 
...
all_complete_Scen4 %>% filter(measure == "Total_Cost")

## added this to get bucket and asset group --------------------------------
projects1 <- read_excel(paste0(path_pp, "/10.04.05 Tactical Works - RFT Addendum 3 (NF Aligned).xlsx"), range = "A1:G512" ,sheet = "delivery year")
all_complete_Scen4 <- all_complete_Scen4 %>% 
  left_join(projects1[,c(1,2,7)], by = c("project_name" = "Project Name")) %>%
  rename(Asset_Group = `Asset Group`) %>% 
  mutate(Asset_Group = ifelse(is.na(Asset_Group),paste0(word(project_name,-1)," asset management"),Asset_Group)) %>% 
  mutate(Asset_Group = ifelse(Asset_Group == "Bridges asset management", "Bridges with no project", 
...
all_complete_Scen4_project <- all_complete_Scen4 %>% select(project,project_name,must,Asset_Group, Bucket) %>%
  rename(name = project_name) %>%
  rename(Zone = Asset_Group) %>%
  rename(Region = Bucket) %>% select(project,name, must,Region, Zone) %>% 
  distinct()
all_complete_Scen4_solution <- all_complete_Scen4 %>% select(project,solution,selection.timing) %>% 
  filter(solution == 1) %>% 
  mutate(time.to.benefits = 1) %>% distinct()
all_complete_Scen4_measures <- all_complete_Scen4 %>% select(project:solution)

write.csv(all_complete_Scen4_project,"C:/Users/TOsosanya/Desktop/NSW/UVO/Scen 4-Project.csv",row.names = F)
write.csv(all_complete_Scen4_solution,"C:/Users/TOsosanya/Desktop/NSW/UVO/Scen 4-Solution.csv",row.names = F)
write.csv(all_complete_Scen4_measures,"C:/Users/TOsosanya/Desktop/NSW/UVO/Scen 4-Measures.csv",row.names = F)

## ends here

openxlsx::write.xlsx(all_complete_Scen4, file = "C:/Users/TOsosanya/Desktop/NSW/Newdata 2705/Pavements/Pavement UVO 3 ids Scen4.xlsx")
