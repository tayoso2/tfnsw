...
up_names_id <- read_excel(("C:/Users/TOsosanya/Desktop/NSW/UVO/Final-NSW.xlsx"),range = "A1:C512", sheet = "id") %>%
  #filter(`Asset Group` != "ITS asset management") %>% 
...
# # Add total cost
# 
# up_names_id2 <- up_names_id %>% 
#   na.omit() %>% 
#   select(project,project_name,Total_Cost) %>% 
...
#          Total_Cost9=0,
#          Total_Cost10=0) %>% 
#   pivot_longer(cols = Total_Cost1:Total_Cost10,
#                names_to = c("measure")
#                #names_pattern = "(.)(.)"
#   ) %>% 
#   as.data.table()
...
#   as.data.frame()
# 
# up_names_id3$measure = gsub('[[:digit:]]+', '', up_names_id3$measure)
# 
# # add solution = 0
# 
# up_names_id4 <- up_names_id3 %>% ungroup() %>% 
...
#   as.data.frame()
# 
# up_names_id5 <- rbind(up_names_id3,up_names_id4) %>% 
#   left_join(up_names_id[,c("project","selection.timing", "must")], by = "project") %>% 
#   distinct()
#   
# all_xx <- rbind(up_names_id[,1:8],up_names_id5)
...
up_names_id_2 <- read_excel(("C:/Users/TOsosanya/Desktop/NSW/UVO/Final-NSW.xlsx"),range = "A1:C517", sheet = "id") %>% 
  #filter(`Asset Group` != "ITS asset management") %>% 
  select(1:2) %>% 
  filter(grepl('Remaining ',`name`))
...
#          value = ifelse(measure == "Selected",1,0)) 
# 
# up_all_x <- rbind(pavement_UP4,pavement_UP5) %>% 
#   mutate(timestep = timestep-3) %>% 
#   distinct() %>% 
#   as.data.table()
# 
# openxlsx::write.xlsx(up_all_x, file = "C:/Users/TOsosanya/Desktop/NSW/Newdata 2705/Pavements/Pavement UVO.xlsx")
