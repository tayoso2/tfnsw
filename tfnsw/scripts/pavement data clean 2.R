...
pma <- projects1_pavement[,c("ObjectID","WKT")] %>% na.omit() %>% as.data.frame()
pma <- st_as_sf(pma,wkt = "WKT")
st_cast(pma, "MULTILINESTRING")
st_write(pma)
...
# output assetbase for EDA
projects1_pavement2 <- projects1_pavement %>% select(Area1, AssetID, Type, Highway, Section, Direction, AvailabilitySegment, PavementClass, ELANES, 
...
st_write(mastersheet, paste0(path_p,"/mastersheet.shp"))

# forest fires
forest_fires <- st_read(paste0(path_p, "/forest_fires.shp"))
forest_fires <- st_sf(forest_fires, crs = 4326)
forest_fires <- st_cast(forest_fires, "LINESTRING")
forest_fires$ObjectID <- seq.int(nrow(forest_fires)) + 100000
forest_fires <- forest_fires[,c("ObjectID","geometry")]
tmap_mode("view")
qtm(na.omit(forest_fires), basemap = "Esri.WorldGrayCanvas") + tm_style("beaver")

st_write(forest_fires,paste0(path_p,"/forest_fires.csv"), layer_options = "GEOMETRY=AS_WKT")

# forest fires
forest_fires2 <- st_read(paste0(path_p, "/forest_fires2.shp"))
forest_fires2 <- st_sf(forest_fires2, crs = 4326)
forest_fires2 <- st_cast(forest_fires2, "LINESTRING")
forest_fires2$ObjectID <- seq.int(nrow(forest_fires2)) + 101000
forest_fires2 <- forest_fires2[,c("ObjectID","geometry")]
tmap_mode("view")
qtm(na.omit(forest_fires2), basemap = "Esri.WorldGrayCanvas") + tm_style("beaver")

st_write(forest_fires2,paste0(path_p,"/forest_fires2.csv"), layer_options = "GEOMETRY=AS_WKT")

# forest fires
forest_fires3 <- st_read(paste0(path_p, "/forest_fires3.shp"))
forest_fires3 <- st_sf(forest_fires3, crs = 4326)
forest_fires3 <- st_cast(forest_fires3, "LINESTRING")
forest_fires3$ObjectID <- seq.int(nrow(forest_fires3)) + 200000
forest_fires3 <- forest_fires3[,c("ObjectID","geometry")]
tmap_mode("view")
qtm(na.omit(forest_fires3), basemap = "Esri.WorldGrayCanvas") + tm_style("beaver")

st_write(forest_fires3,paste0(path_p,"/forest_fires3.csv"), layer_options = "GEOMETRY=AS_WKT")

# forest fires
no_fires <- st_read(paste0(path_p, "/no_fires.shp"))
no_fires <- st_sf(no_fires, crs = 4326)
no_fires <- st_cast(no_fires, "LINESTRING")
no_fires$ObjectID <- seq.int(nrow(no_fires)) + 300000
no_fires <- no_fires[,c("ObjectID","geometry")]
tmap_mode("view")
qtm(na.omit(no_fires), basemap = "Esri.WorldGrayCanvas") + tm_style("beaver")

st_write(no_fires,paste0(path_p,"/no_fires.csv"), layer_options = "GEOMETRY=AS_WKT")
